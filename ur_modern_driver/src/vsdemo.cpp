...
private:
    ...
    {
        //callback for /target_feed topic
        current = c;
    }

    void callback_joint(const sensor_msgs::JointState& state )
    {   //joint space feedback from robot
        cout<<"~~~~~~~~~~~~~~~"<<endl;
        for(int i=0;i<6;i++){
    ...
    {
        /*
        simple features are considered for target tracking and error calculations:
        U[0] = x coordinate of center
        U[1] = y coordinate of center
        U[2] = radius of image in pixels
        */

        ros::init(argc,argv,"ibvs");
        ros::NodeHandle nh;
        //Subscribe to the topic that publishes information about centre coordinates and radius of ball
        ros::Subscriber sub_images = nh.subscribe("target_feed",1000,callback_images);
       ...
       //define target values for image features
        target.x= 320;
        target.y=240;
        target.radius=200;  
        ...
        for(double t = 0; t < TMAX; t = t+dt)
        {
            ...
            //define update to state command using image based error
            sprintf(cmd,"speedj([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f)\n",
                ...
                1.0);
            //send command to robot command queue
